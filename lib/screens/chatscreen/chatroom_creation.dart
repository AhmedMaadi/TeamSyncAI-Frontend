...
class _ChatroomCreationPageState extends State<ChatroomCreationPage> {
  ...
  void _createChatroom(ChatroomProvider provider) async {
    ...
    final String memberEmails = _chatroomMemberEmailController.text.trim();

    if (name.isNotEmpty && topic.isNotEmpty && memberEmails.isNotEmpty) {
      try {
        final userEmail = await provider.getUserEmail(); // Await getUserEmail()
        if (userEmail != null) {
          await provider.createChatroom(name, userEmail,topic);
            ...
            MaterialPageRoute(
              builder: (context) => ChatPage(
                chatRoomModel: ChatroomModel(
                  ...
                  receiverEmail: memberEmails, // Assuming memberEmails is the receiver email
                  topic: topic,
